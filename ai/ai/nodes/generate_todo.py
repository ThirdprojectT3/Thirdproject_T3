...
def generate_todo(state: GenState) -> GenState:
    ...
    if place == "쉬기":
        ...
        instruction += (
            ...
            f"**프롬프트에서 요청한 운동만 추천하세요. 예: '등 운동 루틴'이라면 보디빌더의 등 운동 루틴을 참고해서 추천하세요.**\n"
            f"**프롬프트에서 직접 언급되지 않은 부위나 운동은 절대 포함하지 마세요.**\n"
            f"운동 장소별 가능한 운동 예시: 헬스장(벤치프레스, 데드리프트), 집(맨몸 스쿼트, 푸쉬업, 플랭크, 홈 요가), 크로스핏(월볼샷, 박스점프, 파머스 캐리). "
